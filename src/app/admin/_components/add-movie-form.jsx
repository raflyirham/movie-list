...
function SubmitButton() {
  const { pending } = useFormStatus();
  return <Button type="submit" disabled={pending}>{pending ? "Adding..." : "Add Movie"}</Button>;
}
...
export function AddMovieForm() {
  ...
  return (
    <Dialog onOpenChange={(isOpen) => !isOpen && setGenres([])}>
      <DialogTrigger asChild>
        <Button>
          <PlusCircle className="mr-2 h-4 w-4" /> Add New Movie
        </Button>
        ...
        <form ref={formRef} action={formAction}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="genres" className="text-right pt-2">Genres</Label>
